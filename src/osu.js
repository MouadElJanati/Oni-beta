										...
										ur_promise.then((recent) => {
											...
											tracked_users[
												...
												if (channel)
													channel
														.send(
															`${recent.username} got a new #${recent.pb} top play!`,
															{
																embed,
																files: [
																	{
...
module.exports = {
	...
	format_embed: function (recent) {
		...
		embed.footer = {
			...
			text: `Mapped by ${recent.creator}${
				helper.sep
			}${ranked_text} on ${moment(recent.approved_date).format(
				"D MMMM YYYY"
			)}`,
		};
		embed.thumbnail = {
		...
		try {
			...
			let highcharts_settings = {
				...
				options: {
					...
					subtitle: {
						text: `Версия: ${map.version}, Карту сделал(а) ${map.creator}`,
					},
					yAxis: {
						title: {
							align: "high",
		...
		api.get("/get_user", { params: { u: user } })
			.then((response) => {
				...
				let embed = {
					...
					image: {
						url: `https://images-ext-2.discordapp.net/external/1ZwSLiCu5xCBy3fgjPw3Yjt4giYl7G1jfKbNmTsS0IE/https/assets.ppy.sh/media/generic-header.png`,
					},
					author: {
					...
					footer: {
						text: `Играет уже ${moment(data.join_date).fromNow(
							true
						)}${helper.sep}Зарегестрировался ${moment(
							data.join_date
						).format("D MMMM YYYY")}`,
					},
					fields: [
						...
						{
							name: "Общее количество очков",
							value: Number(data.total_score).toLocaleString(),
							inline: true,
						},
						{
							name: "Всего раз сыгранно",
			...
			.catch((err) => {
				if (err.status == 404) cb("Не могу найти игрока ⭕");
	...
	track_user: function (channel_id, user, top, cb) {
		api.get("/get_user", { params: { u: user } })
				...
				if (response.length > 0) {
					...
					if (user.user_id in tracked_users) {
						if (
							...
							cb(
								`${user.username} is already being tracked in this channel`
							);
						} else {
							...
							cb(
								null,
								`Now tracking ${user.username}'s top ${top} in this channel.`
							);
						}
					} else {
			...
			.catch((err) => {
				if (err.status == 404) cb("Couldn't find user");
				else cb("Couldn't reach osu!api");
					...
					if (user.user_id in tracked_users) {
						if (
							...
							if (
								...
								cb(
									null,
									`Stopped tracking ${user.username} in this channel.`
								);
							} else {
							...
							cb(
								`${user.username} is not being tracked in this channel`
							);
						}
					} else {
