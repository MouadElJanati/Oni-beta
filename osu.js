...
function getMods(enabled_mods) {
	...
	for (var mod in mods_enum) {
		if ((mods_enum[mod] & enabled_mods) != 0) return_array.push(mod);
	}
	return return_array;
}
...
function hasMods(mods, disabledmods, scores) {
	...
	scores.forEach((score) => {
		...
		mods.forEach((mod) => {
			if (!enabled_mods.includes(mod)) check = false;
		});

		disabledmods.forEach((mods) => {
			if (mods.every((a) => enabled_mods.includes(a))) check = false;
		});

		if (check) output.push(score);
	});
...
function getScore(recent_raw, cb) {
	...
	let requests = [
		api.get("/get_user_best", {
			params: { u: recent_raw.user_id, limit: 100 },
		}),
		api.get("/get_scores", {
			params: { b: recent_raw.beatmap_id, limit: 100 },
		}),
		api.get("/get_scores", {
			params: {
				b: recent_raw.beatmap_id,
				u: recent_raw.user_id,
				mods: recent_raw.enabled_mods,
			},
		}),
	...
	Promise.all(requests)
		.then((results) => {
			...
			axios
				.get(`${config.beatmap_api}/b/${recent.beatmap_id}`)
				.then((response) => {
					response = response.data;

					let beatmap = response.beatmap;

					let diff_settings = calculateCsArOdHp(
						beatmap.cs,
						beatmap.ar,
						beatmap.od,
						beatmap.hp,
					...
					if (recent.mods.includes("DT")) speed *= 1.5;
					else if (recent.mods.includes("HT")) speed *= 0.75;
					...
					if (diff.aim && diff.speed) {
						...
						cb(
							"Нету данных сложности по этой карте! Попробуйте попозже."
						);
						return;
					}

					helper
						.downloadBeatmap(recent_raw.beatmap_id)
						.finally(() => {
							let beatmap_path = path.resolve(
								config.osu_cache_path,
								`${recent_raw.beatmap_id}.osu`
							);

							let strains_bar;

							if (fs.existsSync(beatmap_path)) {
								strains_bar = module.exports.get_strains_bar(
									beatmap_path,
									recent.mods.join(""),
									recent.fail_percent
								);

								if (strains_bar) recent.strains_bar = true;
							}

							if (replay && fs.existsSync(beatmap_path)) {
								let ur_promise = new Promise(
									(resolve, reject) => {
										if (config.debug)
										...
										ur_calc.get_ur(
											{
												...
												beatmap_id:
													recent_raw.beatmap_id,
												mods_enabled:
													recent_raw.enabled_mods,
											...
											(ur) => {
												...
												if (
													recent.mods.includes(
														"DT"
													) ||
													recent.mods.includes("NC")
												)
													recent.cvur = ur / 1.5;
												else if (
													recent.mods.includes("HT")
												)
													recent.cvur = ur * 1.5;
				...
				.catch((err) => {
					cb(
						"Карты ещё нету в базе данных. Новеньким картам требуется время что б они работаели стабильно."
					);
					helper.log(err);
					return;
				});
...
function updateTrackedUsers() {
	for (user_id in tracked_users) {
		...
		api.get("/get_user_best", {
			params: { u: user, limit: tracked_users[user].top, mode: 0 },
		})
			.then((response) => {
				response = response.data;
				...
				if (user in top_plays) {
					response.forEach((score) => {
						...
						if (!top_plays[user].includes(Number(score.score_id))) {
							getScore(
								score,
								(err, recent, strains_bar, ur_promise) => {
									if (err) return false;

									if (ur_promise) {
										ur_promise.then((recent) => {
											let embed = module.exports.format_embed(
												recent
											);
											tracked_users[
												user
											].channels.forEach((channel_id) => {
												let channel = discord_client.channels.get(
													channel_id
												);
												if (channel)
													channel
														.send(
															`${recent.username} поставил новый рекорд #${recent.pb}!`,
															{
																embed,
																files: [
																	{
																		attachment: strains_bar,
																		name:
																			"strains_bar.png",
																	},
																],
										...
										tracked_users[user].channels.forEach(
											(channel_id) => {
												let channel = discord_client.channels.get(
													channel_id
												);
...
module.exports = {
	...
	add_pp: function (user, pp_to_add, beatmap, cb) {
		...
		api.get("/get_user", { params: { u: user } }).then((response) => {
			...
			let total_scores =
				parseInt(json[0].count_rank_ss) +
				parseInt(json[0].count_rank_s) +
				parseInt(json[0].count_rank_a);

			api.get("/get_user_best", { params: { u: user, limit: 100 } }).then(
				(response) => {
					json = response.data;
					...
					if (max == 100) {
						...
						json.forEach(function (value, index) {
							if (index >= 90) {
								differences_array.push(
									parseFloat(json[index - 1].pp) -
										parseFloat(json[index].pp)
								);
							}
						});
						...
						for (let x = 0; x < 100; x++) {
							...
							let current_pp_weighted =
								current_pp * current_factor;
					...
					let beta_bonus_pp =
						416.6667 * (1 - Math.pow(0.9994, total_scores));
						...
						if (Number.isInteger(Number(beatmap))) {
							...
							if (index < 0) {
								cb(
									"https://osu.ppy.sh/b/" +
										beatmap +
										": beatmap not found in top scores"
								);
								return false;
							}
							...
							fixing_score =
								" (fixing score on https://osu.ppy.sh/b/" +
								beatmap +
								")";
						}
					} else {
					...
					setTimeout(function () {
						...
						if (no_bonus_pp) {
							output_message +=
								...
								pp_full +
								"pp (#" +
								numberWithCommas(old_rank) +
								") ► +" +
								adding_pp +
								" (" +
								(pp_no_bonus - pp_full).toFixed(1) +
								"pp weighted) ► " +
								numberWithCommas(pp_no_bonus.toFixed(1)) +
								"pp" +
								new_rank +
								" (inactive account, no bonus pp)" +
								fixing_score;
						} else {
							output_message +=
								output_user +
						...
						if (config.debug) {
							...
							helper.log("Результат: " + pp_no_bonus.toFixed(1));
						}
					}, 350);
				}
			);
	...
	format_embed: function (recent) {
		...
		embed.author = {
			...
			name: `${recent.username} – ${
				recent.user_pp
			}pp (#${recent.user_rank.toLocaleString()})`,
		...
		if (recent.pb) embed.description = `**__#${recent.pb} Top Play!__**`;
		...
		let ranked_text = "Отправлено";

		switch (recent.approved) {
			case 1:
				ranked_text = "Ранкнуто";
				break;
			case 2:
				ranked_text = "Подтверждено";
				break;
			case 3:
				ranked_text = "Квалифицировано";
				break;
			case 4:
				ranked_text = "Любимые";
				break;
		...
		embed.footer = {
			...
			text: `Сделал ${recent.creator}${
				helper.sep
			}${ranked_text}  ${moment(recent.approved_date).format(
				"D MMMM YYYY"
			)}`,
		};
		embed.thumbnail = {
		...
		if (recent.lb > 0) lines[0] += `r#${recent.lb}${helper.sep}`;
		...
		if (recent.pp_fc > recent.pp)
			lines[1] += `**${recent.unsubmitted ? "*" : ""}${+recent.pp.toFixed(
				2
			)}pp**${recent.unsubmitted ? "*" : ""} ➔ ${+recent.pp_fc.toFixed(
				2
			)}pp for ${+recent.acc_fc.toFixed(2)}% FC${helper.sep}`;
		else lines[1] += `**${+recent.pp.toFixed(2)}pp**${helper.sep}`;

		if (recent.combo < recent.max_combo)
			lines[1] += `${recent.combo}/${recent.max_combo}x`;
		else lines[1] += `${recent.max_combo}x`;

		if (recent.pp_fc > recent.pp) lines[1] += `\n`;
		else if (
			recent.ur ||
			recent.count100 ||
			recent.count50 ||
		...
		if (recent.count100 > 0) lines[1] += `${recent.count100}x100`;
		...
		if (recent.countmiss > 0) {
			if (recent.count100 > 0 || recent.count50 > 0)
				lines[1] += helper.sep;
		...
		if (recent.ur > 0) {
			...
			if (recent.cvur) lines[1] += ` (${+recent.cvur.toFixed(2)}cv)`;
		} else if (recent.ur < 0) {
			if (
				recent.count100 > 0 ||
				recent.count50 > 0 ||
				...
				lines[1] += helper.sep;
			lines[1] += `xx.xx UR`;
			if (recent.cvur < 0) lines[1] += ` (xx.xxcv)`;
		}

		lines[2] = "Информация о карте";

		lines[3] += `${moment("2015-01-01")
			.startOf("day")
			.seconds(recent.duration)
			.format("mm:ss")} ~ `;
		lines[3] += `CS**${+recent.cs.toFixed(1)}** AR**${+recent.ar.toFixed(
			1
		)}** OD**${+recent.od.toFixed(1)}** HP**${+recent.hp.toFixed(1)}** ~ `;

		if (recent.bpm_min != recent.bpm_max)
			lines[3] += `${+recent.bpm_min.toFixed(
				1
			)}-${+recent.bpm_max.toFixed(1)} (**`;
		else lines[3] += "**";

		lines[3] += +recent.bpm.toFixed(1);

		if (recent.bpm_min != recent.bpm_max) lines[3] += "**)";
		else lines[3] += "**";
	...
	get_recent: function (options, cb) {
		...
		api.get("/get_user_recent", {
			params: { u: options.user, limit: limit },
		}).then((response) => {
			response = response.data;

			if (response.length < 1) {
				cb(`Не найдено недавних игр для ${options.user}`);
				return;
			}
			...
			if (options.pass) {
				...
				for (let i = 0; i < response.length; i++) {
					if (response[i].rank != "F") recent_pass.push(response[i]);
				}
				...
				if (!recent_raw) {
					cb(`Не найдено недавных успешных игр для ${options.user}`);
					return;
				}
			} else {
	...
	get_compare: function (options, cb) {
		...
		if (options.user) {
			...
			if (options.mods) params.mods = getModsEnum(options.mods);
		}
		...
		api.get("/get_scores", { params: params }).then((response) => {
			...
			if (options.mods) {
				...
				filter = response.filter(
					(a) => parseInt(a.enabled_mods) == mods_enum
				);
				...
				if (options.mods[0] == "mods") {
					...
					if (filter.length == 0) {
						...
						if (filter.length == 0) score = response[0];
						else score = filter[0];
					} else {
						score = filter[0];
					}
				...
				if (response.length > 0) score = response[0];
			}
	...
	get_score: function (options, cb) {
		...
		if (options.user) params.u = options.user;
	...
	get_top: function (options, cb) {
		...
		api.get("/get_user_best", { params: params }).then((response) => {
			...
			if (response.length < 1) {
				cb(`Не найдено лучших игр для ${options.user}`);
				return;
			}
			...
			if (options.rb) response = response.sort((a, b) => b.unix - a.unix);

			if (options.ob) response = response.sort((a, b) => a.unix - b.unix);
	...
	get_pp: function (options, cb) {
		axios
			.get(`${config.beatmap_api}/b/${options.beatmap_id}`)
			.then((response) => {
				response = response.data;
				helper.log(response);
				...
				if (options.mods.includes("DT")) speed *= 1.5;
				else if (options.mods.includes("HT")) speed *= 0.75;
				...
				accuracies.forEach((acc) => {
					let pp = ojsama.ppv2(
						Object.assign(pp_calc_obj, { acc_percent: acc })
					);
				...
				accuracies.forEach((acc, index) => {
					if (index > 0) lines[0] += "  ";
					if (acc == custom_acc && options.custom_acc)
						lines[0] += "**";
					lines[0] += `${acc}%`;
					if (acc == custom_acc && options.custom_acc)
						lines[0] += "**";
				...
				pps.forEach((pp, index) => {
					...
					if (
						options.custom_acc &&
						accuracies.indexOf(custom_acc) == index
					)
						lines[1] += "**";
					lines[1] += `${pp}pp`;
					if (
				...
				lines[3] = `CS**${+diff_settings.cs.toFixed(
					1
				)}** AR**${+diff_settings.ar.toFixed(
					1
				)}** OD**${+diff_settings.od.toFixed(
					1
				)}** HP**${+diff_settings.hp.toFixed(1)}** - `;

				if (bpm_min != bpm_max)
					lines[3] += `${+bpm_min.toFixed(1)}-${+bpm_max.toFixed(
						1
					)} (**`;
				else lines[3] += "**";
	...
	parse_beatmap_url: function (beatmap_url, id_only) {
		return new Promise((resolve, reject) => {
			...
			if (id_only === undefined) id_only = false;
			...
			if (beatmap_url.includes("#osu/"))
				...
				beatmap_id = parseInt(
					beatmap_url.split("/discussion/").pop().split("/")[0]
				);
	...
	get_bpm_graph: function (osu_file_path, mods_string, cb) {
		...
		try {
			let parser = new ojsama.parser().feed(
				fs.readFileSync(osu_file_path, "utf8")
			);
			...
			let mods_filtered = mods_array.filter((mod) =>
				TIME_MODS.includes(mod)
			);
			...
			for (let t = 0; t < map.timing_points.length; t++) {
				...
				let bpm = +(
					(MINUTE / timing_point.ms_per_beat) *
					speed_multiplier
				).toFixed(2);
			...
			bpms.push([
				map.objects[map.objects.length - 1].time,
				bpms[bpms.length - 1][1],
			]);
	...
	get_user: function (user, cb) {
		api.get("/get_user", { params: { u: user } })
			.then((response) => {
				response = response.data;
				...
				grades += `${getRankEmoji("XH")} ${Number(
					data.count_rank_ssh
				).toLocaleString()} `;
				grades += `${getRankEmoji("X")} ${Number(
					data.count_rank_ss
				).toLocaleString()} `;
				grades += `${getRankEmoji("SH")} ${Number(
					data.count_rank_sh
				).toLocaleString()} `;
				grades += `${getRankEmoji("S")} ${Number(
					data.count_rank_s
				).toLocaleString()} `;
				grades += `${getRankEmoji("A")} ${Number(
					data.count_rank_a
				).toLocaleString()}`;

				let play_time = `${Math.round(
					Number(data.total_seconds_played) / 3600
				)}h`;
				play_time += ` ${Math.round(
					(Number(data.total_seconds_played) % 3600) / 60
				)}m`;
				...
				let embed = {
					...
					author: {
						name: `${data.username} – ${+Number(
							data.pp_raw
						).toFixed(2)}pp (#${Number(
							data.pp_rank
						).toLocaleString()}) (${data.country}#${Number(
					...
					footer: {
						text: `Играет уже ${moment(data.join_date).fromNow(
							true
						)}${helper.sep}Зарегестрировался(лась) ${moment(
							data.join_date
						).format("D MMMM YYYY")}`,
					},
					fields: [
						{
							name: "Ранкнутое количество очков",
			...
			.catch((err) => {
				if (err.status == 404) cb("Не могу найти пользователя");
				else
					cb(
						"Проблемы с osu!api. Проверьте правильно ли вы ввели токен. Если проблема не в этом, тогда возможно проблема со стороны сервера."
					);
	...
	get_strains_bar: function (osu_file_path, mods_string, progress) {
		let map_strains = module.exports.get_strains(
			osu_file_path,
			mods_string
		);

		if (!map_strains) return false;
	...
	get_strains: function (osu_file_path, mods_string, type) {
		try {
			...
			if (type == "aim") total = stars.aim;

			if (type == "speed") total = stars.speed;

			let aim_strains = calculateStrains(
				1,
				stars.objects,
				speed_multiplier
			);
			let speed_strains = calculateStrains(
				0,
				stars.objects,
				speed_multiplier
			);
			...
			let strain_offset =
				Math.floor(map.objects[0].time / _strain_step) * _strain_step -
				_strain_step;
			...
			for (let i = 0; i < aim_strains.length; i++)
				star_strains.push(
					aim_strains[i] +
						speed_strains[i] +
						Math.abs(speed_strains[i] - aim_strains[i]) *
							EXTREME_SCALING_FACTOR
				);

			let chosen_strains = star_strains;

			if (type == "aim") chosen_strains = aim_strains;

			if (type == "speed") chosen_strains = speed_strains;
			...
			.then((response) => {
				...
				if (response.length > 0) {
					...
					if (user.user_id in tracked_users) {
						if (
							tracked_users[user.user_id].channels.includes(
								channel_id
							)
						) {
							cb(
								`${user.username} уже отслеживается в этом канале`
							);
						} else {
							tracked_users[user.user_id].channels.push(
								channel_id
							);
							...
							cb(
								null,
								`Сейчас отслеживаю ${user.username}, лучшие игры ${top} в этом канале.`
							);
						}
					} else {
						...
						cb(
							null,
							`Сейчас отслеживаю ${user.username}, лучшая игра ${top}.`
						);
					}

					helper.setItem(
						"tracked_users",
						JSON.stringify(tracked_users)
					);
					helper.setItem("top_plays", JSON.stringify(top_plays));
				} else {
					cb(`Не могу найти пользователя \`${user}\``);
				}
			})
			.catch((err) => {
				if (err.status == 404) cb("Не могу найти пользователя");
				else cb("Couldn't reach osu!api");
				...
				if (response.length > 0) {
					...
					if (user.user_id in tracked_users) {
						if (
							...
							tracked_users[
								user.user_id
							].channels = tracked_users[
								user.user_id
							].channels.filter((a) => a != channel_id);

							if (
								tracked_users[user.user_id].channels.length > 0
							) {
								cb(
									null,
									`Я прекратила отслеживать ${user.username} в этом канале.`
								);
							} else {
								cb(null, `Прекращаю слежку ${user.username}.`);
							...
							helper.setItem(
								"top_plays",
								JSON.stringify(top_plays)
							);
						} else {
							cb(
								`${user.username} не отслеживается в этом канале`
							);
						}
					} else {
						cb(`${user.username} не отслеживается`);
					}
				} else {
					cb(`Не могу найти \`${user}\``);
				}
			})
			.catch((err) => {
				if (err.status == 404) cb("Не могу найти пользователя");
		...
		try {
			...
			let highcharts_settings = {
				...
				options: {
					...
					title: { text: `${map.artist} - ${map.title}` },
					subtitle: {
						text: `Версия: ${map.version}, Сделал(а) ${map.creator}`,
					},
					yAxis: {
						title: {
							align: "high",
			...
			highcharts.export(highcharts_settings, (err, res) => {
				if (err) cb("An error occured creating the graph");
					...
					frame.get_frame(
						osu_file_path,
						max_strain_time_real - (map.objects[0].time % 400),
						mods_array,
						[468, 351],
						{ ar: ar, cs: cs },
						(err, output_frame) => {
							if (err) cb(err);
								...
								Jimp.read(Buffer.from(res.data, "base64"))
									.then((_graph) => {
										Jimp.read(output_frame)
											.then((_frame) => {
