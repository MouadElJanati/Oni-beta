...
module.exports = {
	...
	log: (...params) => {
		console.log(`OSU Module:`, ...params);
	},

	error: (...params) => {
		console.error(`[${moment().format("LLLL")}]`, ...params);
	},
	...
	commandHelp: (command_name) => {
		if (Array.isArray(command_name)) command_name = command_name[0];
		...
		for (let i = 0; i < commands.length; i++) {
			...
			if (command.command.includes(command_name)) {
				...
				if (command.command.length > 1) commands_name += "s";

				command.command.forEach((_command, index) => {
					if (index > 0) commands_value += ", ";
				...
				if (command.example) {
					...
					if (examples.length > 1) examples_name += "s";

					examples.forEach((example, index) => {
						if (index > 0) examples_value += "\n\n";
	...
	downloadBeatmap: (beatmap_id) => {
		return new Promise((resolve, reject) => {
			...
			if (
				!fs.existsSync(beatmap_path) ||
				(fs.existsSync(beatmap_path) &&
					!module.exports.validateBeatmap(beatmap_path))
			) {
				module.exports
					.downloadFile(
						beatmap_path,
	...
	getUsername: (args, message, user_ign) => {
		...
		args.forEach(function (arg) {
			if (module.exports.validUsername(arg)) return_username = arg;
			else if (
				module.exports.validUsername(arg.substr(1)) &&
				arg.startsWith("*")
			)
				return_username = arg;
		});
		...
		if (message.guild && user_ign) {
			...
			args.forEach(function (arg) {
				...
				members.forEach((member) => {
					if (
						module.exports.simplifyUsername(member.user.username) ==
						module.exports.simplifyUsername(arg)
					)
						matching_members.push(member.id);
				});

				matching_members.forEach((member) => {
					if (member in user_ign) return_username = user_ign[member];
				});
			});
		}
