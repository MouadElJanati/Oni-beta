...
module.exports = {
	command: "bpm",
	description: "Показать визуальный график BPM с течением времени для карты.",
	usage: "[beatmap url] [+mods]",
	example: [
		{
			run: "bpm",
			result: "Возвращает график BPM для последней карты.",
		},
		{
			run: "bpm https://osu.ppy.sh/b/75 +DT",
			result: "Возвращает график BPM с DT для определенной карты.",
		},
	],
	...
	call: (obj) => {
		return new Promise((resolve, reject) => {
			...
			argv.slice(1).forEach((arg) => {
				if (arg.startsWith("+")) mods = arg.toUpperCase().substr(1);
				else {
					beatmap_url = arg;
			...
			Promise.resolve(beatmap_promise).then(() => {
				...
				if (!beatmap_id) {
					...
					download_path = path.resolve(
						os.tmpdir(),
						`${Math.floor(Math.random() * 1000000) + 1}.osu`
					);

					download_promise = helper.downloadFile(
						download_path,
						download_url
					);
